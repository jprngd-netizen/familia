    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto space-y-6 lg:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3 sm:gap-4">
          <button onClick={onBack} className="p-2 hover:bg-norton-cardHover rounded-full transition text-gray-400">
            <ArrowLeft size={20} className="sm:w-6 sm:h-6" />
          </button>
          <div className="flex items-center gap-3 sm:gap-4">
            <img src={child.avatar} className="w-12 h-12 sm:w-16 sm:h-16 rounded-xl sm:rounded-2xl border-2 sm:border-4 border-norton-border shadow-md" alt="" />
            <div>
        ...
        <div className="flex gap-2 sm:gap-4 w-full sm:w-auto">
          <button
            ...
            className="flex-1 sm:flex-none px-3 sm:px-6 py-2 sm:py-3 rounded-xl sm:rounded-2xl font-bold flex items-center justify-center gap-2 bg-norton-danger/20 text-norton-danger hover:bg-norton-danger/30 transition-all border border-norton-danger/30 text-sm sm:text-base"
          >
          ...
          <button
            ...
            className={`flex-1 sm:flex-none px-3 sm:px-6 py-2 sm:py-3 rounded-xl sm:rounded-2xl font-bold flex items-center justify-center gap-2 transition-all text-sm sm:text-base ${isAdding ? 'bg-norton-cardHover text-gray-400 border border-norton-border' : 'bg-norton-yellow text-norton-dark shadow-lg shadow-norton-yellow/20 hover:bg-norton-gold'}`}
          >
      ...
      {isAdding && (
        <div className="bg-norton-card p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-[2.5rem] border-2 border-norton-yellow/30 shadow-xl animate-in zoom-in duration-300">
          <h3 className="text-lg sm:text-xl font-bold text-white mb-4 sm:mb-6 flex items-center gap-2">
            <Zap className="text-norton-yellow" size={20} /> Configurar Nova Missao
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
            <div className="sm:col-span-2 xl:col-span-2">
              ...
              <input
                ...
                className="w-full bg-norton-dark border border-norton-border rounded-xl sm:rounded-2xl px-4 sm:px-6 py-3 sm:py-4 font-bold outline-none focus:border-norton-yellow transition text-white text-sm sm:text-base"
              />
            </div>
            <div>
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-norton-dark border border-norton-border rounded-xl sm:rounded-2xl px-4 sm:px-6 py-3 sm:py-4 font-bold outline-none appearance-none cursor-pointer text-white text-sm sm:text-base"
              >
          ...
          <div className="mt-6 sm:mt-8 flex justify-end">
            <button
              ...
              className="w-full sm:w-auto bg-norton-yellow text-norton-dark px-6 sm:px-10 py-3 sm:py-4 rounded-xl sm:rounded-2xl font-black shadow-lg hover:bg-norton-gold transition active:scale-95 uppercase tracking-widest text-xs sm:text-sm"
            >
              Confirmar Missao
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 lg:gap-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 xl:grid-cols-1 gap-4 sm:gap-6">
          <div className="bg-norton-card p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-[2.5rem] shadow-sm border border-norton-border">
            ...
            <div className={`p-4 sm:p-6 rounded-2xl sm:rounded-3xl mb-4 sm:mb-6 flex flex-col items-center justify-center text-center border-2 transition-all ${child.unlockedHours > 0 ? 'bg-norton-success/10 border-norton-success/30' : 'bg-norton-danger/10 border-norton-danger/30'}`}>
              <div className={`p-3 sm:p-4 rounded-full mb-3 sm:mb-4 ${child.unlockedHours > 0 ? 'bg-norton-success text-white' : 'bg-norton-danger text-white'}`}>
                {child.unlockedHours > 0 ? <Unlock size={24} className="sm:w-8 sm:h-8" /> : <Lock size={24} className="sm:w-8 sm:h-8" />}
              </div>
              <p className={`text-lg sm:text-2xl font-black ${child.unlockedHours > 0 ? 'text-norton-success' : 'text-norton-danger'}`}>
            ...
            <button
              ...
              className="w-full bg-norton-yellow text-norton-dark py-3 sm:py-4 rounded-xl sm:rounded-2xl font-bold hover:bg-norton-gold shadow-lg shadow-norton-yellow/20 transition active:scale-95 flex items-center justify-center gap-2 text-sm sm:text-base"
            >
              <Zap size={16} className="sm:w-[18px] sm:h-[18px]" /> +1h de Desbloqueio
            </button>
          </div>

          <div className="bg-gradient-to-br from-norton-dark to-norton-darker p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-[2.5rem] text-white shadow-xl border border-norton-border">
             <div className="flex justify-between items-start mb-4 sm:mb-6">
                <Coins size={24} className="sm:w-8 sm:h-8 text-norton-yellow" />
                <span className="bg-norton-yellow/10 border border-norton-yellow/20 px-2 sm:px-3 py-1 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-norton-yellow">Cofrinho</span>
             </div>
             ...
             <div className="p-3 sm:p-4 bg-norton-card rounded-xl sm:rounded-2xl border border-norton-border">
                <p className="text-[10px] sm:text-xs font-medium text-gray-400">Equivalente:</p>
                <p className="text-lg sm:text-xl font-bold text-norton-yellow">{(child.points / 100).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p>
             </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-orange-500/20 to-amber-500/20 p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-[2.5rem] text-white shadow-xl border border-orange-500/30">
            ...
            <div className="p-3 sm:p-4 bg-norton-card rounded-xl sm:rounded-2xl border border-orange-500/20 flex items-center gap-3">
          ...
          <div className="bg-norton-card p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-[2.5rem] shadow-sm border border-norton-border">
             <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6 sm:mb-8">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="w-24 sm:w-32 h-2 bg-norton-border rounded-full overflow-hidden">
                    <div className="bg-norton-yellow h-full transition-all" style={{ width: `${progress}%` }} />
                  </div>
                </div>
             </div>
             ...
             <div className="space-y-3 sm:space-y-4">
                {child.tasks.map(task => (
                  <div key={task.id} className={`flex flex-col sm:flex-row sm:items-center justify-between p-4 sm:p-5 rounded-2xl sm:rounded-3xl border transition-all gap-3 ${task.completed ? 'bg-norton-dark/50 border-norton-border opacity-60' : 'bg-norton-dark border-norton-border hover:border-norton-yellow/30 hover:shadow-md'}`}>
                    <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                      <button
                        onClick={() => onToggleTask(child.id, task.id)}
                        className={`p-1 rounded-lg transition flex-shrink-0 ${task.completed ? 'text-norton-success' : 'text-gray-600 hover:text-norton-yellow'}`}
                      >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-2 sm:gap-3 mt-1 flex-wrap">
                          {task.recurrence && task.recurrence !== 'none' && (
                            <span className="flex items-center gap-1 text-[9px] font-black uppercase text-norton-yellow bg-norton-yellow/10 px-2 py-0.5 rounded-full">
                              <Repeat size={10} /> {getRecurrenceLabel(task.recurrence)}
                            </span>
                          )}
                          {task.schedule?.end && !task.completed && (
                            <span className="flex items-center gap-1 text-[9px] font-black uppercase text-gray-500 bg-norton-border px-2 py-0.5 rounded-full">
                          ...
                          {(() => {
                            ...
                            const colors: Record<string, string> = {
                              overdue: 'text-norton-danger bg-norton-danger/20 animate-pulse',
                              urgent: 'text-norton-danger bg-norton-danger/20',
                              warning: 'text-norton-gold bg-norton-gold/20'
                            };
                            return (
                          ...
                          {task.points === 0 && !task.completed && (
                            <span className="flex items-center gap-1 text-[9px] font-black uppercase text-norton-gold bg-norton-gold/20 px-2 py-0.5 rounded-full">
                    ...
                    <div className="flex items-center gap-2 sm:gap-4 ml-auto sm:ml-0">
                      {editingTaskId === task.id ? (
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            className="w-16 sm:w-20 bg-norton-dark border border-norton-border rounded-lg px-2 py-1 font-bold text-norton-yellow text-sm"
                          />
                          <button onClick={savePoints} className="text-norton-success hover:bg-norton-success/20 p-1 rounded-lg transition">
                            <Save size={16} className="sm:w-[18px] sm:h-[18px]" />
                          </button>
                        </div>
                      ) : (
                        <div className="flex items-center gap-1 sm:gap-2">
                          <span className={`text-xs sm:text-sm font-black ${task.points === 0 ? 'text-norton-gold animate-pulse' : 'text-norton-yellow'}`}>
                            {task.points > 0 ? `+${task.points}` : 'Definir'}
                          </span>
                          <button onClick={() => startEditingPoints(task)} className="text-gray-600 hover:text-norton-yellow transition p-1">
                      ...
                      <button
                        ...
                        className="text-gray-600 hover:text-norton-danger transition p-1"
                      >
             ...
             <div className="space-y-3 sm:space-y-4">
                {[
                  ...
                  <div key={i} className="flex gap-3 sm:gap-4 items-start">
                    <div className="w-2 h-2 mt-1.5 rounded-full bg-norton-yellow flex-shrink-0" />
                    <div>
